...
/* 
   参数:
   ip_ptr - 指向 NX_IP 结构的指针，用于标识 IP 实例。
   
   返回值:
   成功时返回 NX_SUCCESS。失败时返回其他错误代码。
*/
...
/* 
   参数:
   ip_ptr - 指向 NX_IP 结构的指针，用于标识 IP 实例。
   pings_sent - 指向接收已发送 Ping 请求数量的 ULONG 指针。
   ping_timeouts - 指向接收 Ping 超时数量的 ULONG 指针。
   ping_threads_suspended - 指向接收因等待 Ping 响应而挂起的线程数量的 ULONG 指针。
   ping_responses_received - 指向接收已接收的 Ping 响应数量的 ULONG 指针。
   icmp_checksum_errors - 指向接收 ICMP 校验和错误数量的 ULONG 指针。
   icmp_unhandled_messages - 指向接收未处理的 ICMP 消息数量的 ULONG 指针。
...
/* 发送 ICMP Ping 请求并接收响应。 */
/* 
   参数:
   ip_ptr - 指向 NX_IP 结构的指针，用于标识 IP 实例。
   ip_address - 目标 IP 地址。
   data - 指向要发送的 Ping 数据的指针。
   data_size - 要发送的数据大小，以字节为单位。
   response_ptr - 指向接收响应包的 NX_PACKET 指针。
   wait_option - 指定等待选项，表示等待响应的时间。
...
/* 使用扩展的 ICMP 协议（支持 IPv6 地址）发送 Ping 请求并接收响应。 */
/* 
   参数:
   ip_ptr - 指向 NX_IP 结构的指针，用于标识 IP 实例。
   ip_address - 指向目标 NXD_ADDRESS 结构的指针，用于标识目标 IP 地址（可以是 IPv4 或 IPv6）。
   data_ptr - 指向要发送的 Ping 数据的指针。
   data_size - 要发送的数据大小，以字节为单位。
   response_ptr - 指向接收响应包的 NX_PACKET 指针。
   wait_option - 指定等待选项，表示等待响应的时间。
...
/* 通过指定的源地址发送扩展 ICMP Ping 请求并接收响应。 */
/* 
   参数:
   ip_ptr - 指向 NX_IP 结构的指针，用于标识 IP 实例。
   ip_address - 指向目标 NXD_ADDRESS 结构的指针，用于标识目标 IP 地址（可以是 IPv4 或 IPv6）。
   address_index - 源地址索引，用于指定发送 Ping 请求的源地址。
   data_ptr - 指向要发送的 Ping 数据的指针。
   data_size - 要发送的数据大小，以字节为单位。
   response_ptr - 指向接收响应包的 NX_PACKET 指针。
   wait_option - 指定等待选项，表示等待响应的时间。
...
/* 设置 ICMPv6 RA 标志的回调函数。 */
/* 
   参数:
   ip_ptr - 指向 NX_IP 结构的指针，用于标识 IP 实例。
   icmpv6_ra_flag_callback - 指向回调函数的指针，当接收到 RA 报文时会调用此函数，并传递 RA 标志。
