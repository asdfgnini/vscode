...
//#define touch_printf(...) 
/* 定义触笔中断INT的GPIO端口 */
#define TP_INT_GPIO_CLK_ENABLE()	__HAL_RCC_GPIOG_CLK_ENABLE()
	...
	{		
		...
		*/
		if (i2c_CheckDevice(0x28) == 0)
		{
			...
			else if(id == 0x37343139) //三合一屏幕
			{
				uint16_t MaxX, MaxY;
				GT911_ReadMaxXY(&MaxX, &MaxY);
				g_TouchType = CT_GT9147;
				g_LcdType = LCD_43_800X480;		
				printf("Y:%d  X:%d \r\n",MaxY,MaxX);
				touch_printf("检测到4.3寸电容触摸屏GT9147(0x28) 800x480\r\n");
			}
			else 	/* GT811 */
			{
		...
		if (i2c_CheckDevice(0xBA) == 0)
		{
			...
			{			
				...
				if (MaxX == 480 && MaxY == 272)
				{
					g_TouchType = CT_GT911;
					g_LcdType = LCD_43_480X272;		
					touch_printf("检测到4.3寸电容触摸屏GT911(0xBA) 480x272\r\n");
				}				
				else
				{
					g_GT911.i2c_addr = 0xBA;
			...
			else if(id == 0x37343139) //三合一屏幕
			{
				...
				g_LcdType = LCD_43_800X480;		
				printf("Y:%d  X:%d \r\n",MaxY,MaxX);
				touch_printf("检测到4.3寸电容触摸屏GT9147(0xBA) 800x480\r\n");
			}
			else if(id == 0x37353139)// 野火5寸 800x480
			{
				uint16_t MaxX, MaxY;
				GT911_ReadMaxXY(&MaxX, &MaxY);
				g_TouchType = CT_GT9147;
				...
				printf("Y:%d  X:%d \r\n",MaxY,MaxX);
				touch_printf("检测到5.0寸电容触摸屏GT9157(0xBA) 800x480\r\n");
			}
			else 	/* GT811 */
			{
...
*********************************************************************************************************
*	函 数 名: TOUCH_Scan
*	功能说明: 电阻触摸板事件检测程序。该函数被周期性调用，每ms调用1次. 见 bsp_Timer.c
*	形    参:  无
*	返 回 值: 无
*********************************************************************************************************
*/
